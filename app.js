...
const state = {
  ...
  ride:{status:'ready',watchId:null,startTs:0,pauseStarted:0,pausedMs:0,points:[],distanceM:0,ascentM:0,maxSpeedKmh:0,lastAccepted:null,timer:null,wakeLock:null,followPauseUntil:0,followTimer:null},
  navVoice:{enabled:false,announced:{},offRouteAt:0,wasOffRoute:false,arrival:false},
  modalRide:null,
};
...
function switchTab(name){
  ...
  document.body.classList.toggle('ride-mode',name==='ride');
...
function initMaps(){
  ...
  state.rideMap.on('dragstart',()=>{if(state.ride.status==='recording')pauseRideMapFollow();});
  ...
  state.routeMap.on('click',e=>{
    ...
    if(!state.mapPick)return;setMapDestination(e.latlng.lat,e.latlng.lng,'tap');
  });
}

let routeDestinationMarker=null, routeOriginMarker=null, homeMarker=null;
function clearRoutePreviewForDestinationChange(){state.routeData=null;state.routeLayers.forEach(l=>l.remove());state.routeLayers=[];els.routeResult.hidden=true;els.fitRouteBtn.textContent='現在地';}
function setMapDestination(lat,lng,source='tap'){const y=Number(lat),x=Number(lng);if(!Number.isFinite(y)||!Number.isFinite(x))return;clearRoutePreviewForDestinationChange();state.destinationPoint={lat:y,lng:x,label:'地図で指定'};els.destination.value='地図で指定';state.mapPick=false;els.mapPickBtn.classList.remove('active');els.mapPickBtn.textContent='目的地を変更';els.destinationHint.textContent=source==='drag'?'目的地を移動しました。ルートを再作成してください。':'地図上の目的地を設定しました。もう一度変更できます。';els.destinationHint.classList.remove('active');clearPlaceSelection();drawDestinationMarker();}
function drawDestinationMarker(){if(!state.routeMap||!state.destinationPoint)return;if(routeDestinationMarker)routeDestinationMarker.remove();routeDestinationMarker=L.marker([state.destinationPoint.lat,state.destinationPoint.lng],{draggable:true,autoPan:true,title:'目的地・ドラッグで変更'}).bindTooltip('目的地・ドラッグで変更',{direction:'top'}).addTo(state.routeMap);routeDestinationMarker.on('dragend',e=>{const ll=e.target.getLatLng();setMapDestination(ll.lat,ll.lng,'drag');});}
...
const HOME_KEY='yoshiRideHomeV1', PLAN_KEY='yoshiRideActivePlanV1', VOICE_KEY='yoshiRideVoiceV1';
...
els.mapPickBtn.addEventListener('click',()=>{state.mapPick=!state.mapPick;els.mapPickBtn.classList.toggle('active',state.mapPick);els.mapPickBtn.textContent=state.mapPick?'地図をタップ':(state.destinationPoint?'目的地を変更':'地図で指定');els.destinationHint.textContent=state.mapPick?'地図上の新しい目的地をタップしてください。マーカーをドラッグして変更することもできます。':(state.destinationPoint?'目的地はいつでも変更できます。':'下の地図をタップして目的地を指定することもできます。');els.destinationHint.classList.toggle('active',state.mapPick);if(state.mapPick&&state.routeMap){state.routeMap.getContainer().scrollIntoView({behavior:'smooth',block:'center'});}});
els.destination.addEventListener('input',()=>{if(els.destination.value!=='地図で指定'){state.destinationPoint=null;clearPlaceSelection();clearRoutePreviewForDestinationChange();if(routeDestinationMarker){routeDestinationMarker.remove();routeDestinationMarker=null}els.mapPickBtn.textContent='地図で指定';}});
...
async function calculateRoute(){
  ...
  try{
    ...
    state.routeData=data;state.destinationPoint=data.destination;els.destination.value=data.destination.label||els.destination.value;els.mapPickBtn.textContent='目的地を変更';renderRoute(data);setPlannedRoute(data,false);setMessage('ルートを作成しました。RIDEにも反映済みです。','success');
...
const TURN_TYPES={
  0:{icon:'\u2190',text:'\u5de6\u6298',speech:'\u5de6\u6298\u3067\u3059'},
  1:{icon:'\u2192',text:'\u53f3\u6298',speech:'\u53f3\u6298\u3067\u3059'},
  2:{icon:'\u21b0',text:'\u92ed\u304f\u5de6\u6298',speech:'\u92ed\u304f\u5de6\u6298\u3067\u3059'},
  3:{icon:'\u21b1',text:'\u92ed\u304f\u53f3\u6298',speech:'\u92ed\u304f\u53f3\u6298\u3067\u3059'},
  4:{icon:'\u2196',text:'\u5de6\u659c\u3081',speech:'\u5de6\u659c\u3081\u65b9\u5411\u3067\u3059'},
  5:{icon:'\u2197',text:'\u53f3\u659c\u3081',speech:'\u53f3\u659c\u3081\u65b9\u5411\u3067\u3059'},
  7:{icon:'\u21bb',text:'\u30ed\u30fc\u30bf\u30ea\u30fc',speech:'\u30ed\u30fc\u30bf\u30ea\u30fc\u306b\u9032\u5165\u3057\u307e\u3059'},
  8:{icon:'\u21b1',text:'\u30ed\u30fc\u30bf\u30ea\u30fc\u51fa\u53e3',speech:'\u30ed\u30fc\u30bf\u30ea\u30fc\u3092\u51fa\u307e\u3059'},
  9:{icon:'\u21b6',text:'U\u30bf\u30fc\u30f3',speech:'U\u30bf\u30fc\u30f3\u3067\u3059'},
  10:{icon:'\u25cf',text:'GOAL',speech:'\u76ee\u7684\u5730\u3067\u3059'},
  12:{icon:'\u2199',text:'\u5de6\u65b9\u5411\u3092\u7dad\u6301',speech:'\u5de6\u65b9\u5411\u3092\u7dad\u6301\u3057\u307e\u3059'},
  13:{icon:'\u2198',text:'\u53f3\u65b9\u5411\u3092\u7dad\u6301',speech:'\u53f3\u65b9\u5411\u3092\u7dad\u6301\u3057\u307e\u3059'},
};
function resetNavVoiceAlerts(){state.navVoice.announced={};state.navVoice.offRouteAt=0;state.navVoice.wasOffRoute=false;state.navVoice.arrival=false;}
function updateVoiceButton(){if(!els.voiceGuideBtn)return;const supported='speechSynthesis'in window&&'SpeechSynthesisUtterance'in window;els.voiceGuideBtn.disabled=!supported;els.voiceGuideBtn.textContent=supported?(state.navVoice.enabled?'\u97f3\u58f0 ON':'\u97f3\u58f0 OFF'):'\u97f3\u58f0 --';els.voiceGuideBtn.classList.toggle('voice-on',supported&&state.navVoice.enabled);els.voiceGuideBtn.setAttribute('aria-pressed',supported&&state.navVoice.enabled?'true':'false');}
function loadVoicePreference(){try{state.navVoice.enabled=localStorage.getItem(VOICE_KEY)==='1'}catch{}updateVoiceButton();}
function speakNav(text,force=false){if(!state.navVoice.enabled||!('speechSynthesis'in window)||!('SpeechSynthesisUtterance'in window))return;if(!force&&state.ride.status!=='recording')return;try{window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(String(text));u.lang='ja-JP';u.rate=1.05;u.pitch=1;u.volume=1;window.speechSynthesis.speak(u);}catch{}}
function turnInfo(type){return TURN_TYPES[Number(type)]||{icon:'\u2191',text:'\u9032\u884c',speech:'\u305d\u306e\u307e\u307e\u9032\u307f\u307e\u3059'};}
function formatNavDistance(m){const d=Math.max(0,Number(m||0));if(d<100)return `${Math.round(d/10)*10} m`;if(d<1000)return `${Math.round(d/50)*50} m`;return `${(d/1000).toFixed(d<10000?1:0)} km`;}
function scaledManeuverProgress(m,plan,prog){const raw=Number(m?.progressM||0),summary=Number(plan?.summary?.distanceM||0);return summary>0?raw*(prog.totalM/summary):raw;}
function nextManeuver(plan,prog,progressM){const list=Array.isArray(plan?.maneuvers)?plan.maneuvers:[];let best=null;for(const m of list){const type=Number(m?.type);if(type===11||type===6)continue;const at=scaledManeuverProgress(m,plan,prog);if(at<progressM-35)continue;if(!best||at<best.at)best={m,at};}return best;}
function setTurnCard(icon,distance,text,mode=''){if(!els.rideTurnCard)return;els.rideTurnIcon.textContent=icon;els.rideTurnDistance.textContent=distance;els.rideTurnInstruction.textContent=text;els.rideTurnCard.classList.toggle('offroute',mode==='offroute');els.rideTurnCard.classList.toggle('arrived',mode==='arrived');}
function maneuverPhrase(m){const info=turnInfo(m?.type);const name=String(m?.name||'').trim();return name?`${info.speech}\u3001${name}\u65b9\u9762\u3078`:info.speech;}
function maybeAnnounceManeuver(m,distanceM){if(state.ride.status!=='recording'||!state.navVoice.enabled||Number(m?.type)===10)return;const key=`${Number(m?.type)}:${Number(m?.progressM||0)}:${String(m?.name||'')}`;const done=state.navVoice.announced[key]||(state.navVoice.announced[key]={});const phrase=maneuverPhrase(m);if(distanceM<=25&&!done.now){done.m200=true;done.m50=true;done.now=true;speakNav(`\u307e\u3082\u306a\u304f\u3001${phrase}`);return;}if(distanceM<=70&&!done.m50){done.m200=true;done.m50=true;speakNav(`50\u30e1\u30fc\u30c8\u30eb\u5148\u3001${phrase}`);return;}if(distanceM<=230&&!done.m200){done.m200=true;speakNav(`200\u30e1\u30fc\u30c8\u30eb\u5148\u3001${phrase}`);}}
function updateTurnGuidance(plan,prog,progressM,remainingM,off){if(!plan||!prog){setTurnCard('\u2191','--','\u30eb\u30fc\u30c8\u3092\u4f5c\u6210\u3059\u308b\u3068\u66f2\u304c\u308a\u6848\u5185\u3092\u8868\u793a\u3057\u307e\u3059');return;}if(off){setTurnCard('!','ROUTE OFF','\u30eb\u30fc\u30c8\u3078\u623b\u3063\u3066\u304f\u3060\u3055\u3044','offroute');if(state.ride.status==='recording'&&state.navVoice.enabled){const now=Date.now();if(!state.navVoice.wasOffRoute||now-state.navVoice.offRouteAt>45000){state.navVoice.offRouteAt=now;speakNav('\u30eb\u30fc\u30c8\u304b\u3089\u5916\u308c\u3066\u3044\u307e\u3059');}}state.navVoice.wasOffRoute=true;return;}if(state.navVoice.wasOffRoute&&state.ride.status==='recording'&&state.navVoice.enabled)speakNav('\u30eb\u30fc\u30c8\u306b\u623b\u308a\u307e\u3057\u305f');state.navVoice.wasOffRoute=false;if(remainingM<=45){setTurnCard('\u25cf','GOAL','\u76ee\u7684\u5730\u306b\u5230\u7740\u3057\u307e\u3059','arrived');if(state.ride.status==='recording'&&state.navVoice.enabled&&!state.navVoice.arrival){state.navVoice.arrival=true;speakNav('\u76ee\u7684\u5730\u306b\u5230\u7740\u3057\u307e\u3057\u305f');}return;}const next=nextManeuver(plan,prog,progressM);if(!next){const has=Array.isArray(plan?.maneuvers)&&plan.maneuvers.length>0;setTurnCard('\u2191','--',has?'\u3053\u306e\u5148\u306f\u76ee\u7684\u5730\u307e\u3067\u9053\u306a\u308a\u3067\u3059':'\u30eb\u30fc\u30c8\u3092\u518d\u4f5c\u6210\u3059\u308b\u3068\u66f2\u304c\u308a\u6848\u5185\u304c\u4f7f\u3048\u307e\u3059');return;}const distanceM=Math.max(0,next.at-progressM),info=turnInfo(next.m.type),name=String(next.m.name||'').trim();setTurnCard(info.icon,formatNavDistance(distanceM),name?`${info.text} \u30fb ${name}`:info.text);maybeAnnounceManeuver(next.m,distanceM);}
...
function updatePlanNavigation(p){
  const plan=state.plannedRoute,prog=state.planProgress;
  if(!plan||!prog||!p){
    els.rideDestination.textContent=plan?.destination?.label||'\u30d5\u30ea\u30fc\u30e9\u30a4\u30c9';els.rideRemaining.textContent='-- km';els.rideArrival.textContent='--:--';els.rideRouteState.textContent=plan?'GPS\u5f85\u6a5f\u4e2d':'\u30eb\u30fc\u30c8\u672a\u8a2d\u5b9a';els.rideRouteState.classList.remove('offroute');els.rideUpcomingGrade.textContent='-- %';els.rideNextClimb.textContent='\u30eb\u30fc\u30c8\u3092\u8a2d\u5b9a\u3059\u308b\u3068\u8868\u793a\u3057\u307e\u3059';updateTurnGuidance(plan,prog,0,Infinity,false);return;
  }
  const pts=plan.route||[];let bestI=0,bestD=Infinity;const last=prog.lastIndex||0;const lo=Math.max(0,last-120),hi=Math.min(pts.length,last+260);
  for(let i=lo;i<hi;i++){const d=haversine(p,pts[i]);if(d<bestD){bestD=d;bestI=i}}
  if(bestD>800){for(let i=0;i<pts.length;i+=4){const d=haversine(p,pts[i]);if(d<bestD){bestD=d;bestI=i}}}
  prog.lastIndex=bestI;const progressM=prog.cum[bestI]||0,remainingM=Math.max(0,prog.totalM-progressM);els.rideDestination.textContent=plan.destination?.label||'\u76ee\u7684\u5730';els.rideRemaining.textContent=`${(remainingM/1000).toFixed(1)} km`;
  let speedMps=0;if(state.ride.status!=='ready'){const elapsed=currentElapsed();if(elapsed>60&&state.ride.distanceM>300)speedMps=state.ride.distanceM/elapsed;}
  const remainS=speedMps>2?remainingM/speedMps:Number(plan.summary?.durationS||0)*(remainingM/Math.max(1,prog.totalM));els.rideArrival.textContent=clockTimeFromNow(remainS);
  const off=bestD>140;els.rideRouteState.textContent=off?`\u30eb\u30fc\u30c8\u304b\u3089\u7d04 ${Math.round(bestD)}m \u96e2\u308c\u3066\u3044\u307e\u3059`:`ROUTE ON \u30fb ${Math.round(progressM/100)/10}km\u5730\u70b9`;els.rideRouteState.classList.toggle('offroute',off);
  let ahead=bestI;while(ahead<pts.length-1&&(prog.cum[ahead]-progressM)<150)ahead++;const a=pts[bestI],b=pts[ahead];const dist=Math.max(1,(prog.cum[ahead]||progressM)-progressM);const grade=Number.isFinite(a?.ele)&&Number.isFinite(b?.ele)?((b.ele-a.ele)/dist*100):0;els.rideUpcomingGrade.textContent=`${grade.toFixed(1)} %`;
  const climbs=plan.climbs||[];const next=climbs.filter(c=>Number(c.endKm)*1000>=progressM).sort((x,y)=>Number(x.startKm)-Number(y.startKm))[0];
  if(next){const toStart=Math.max(0,Number(next.startKm)*1000-progressM);els.rideNextClimb.textContent=toStart<150?`${Number(next.distanceKm).toFixed(1)}km / \u5e73\u5747 ${Number(next.avgGrade).toFixed(1)}% / MAX ${Number(next.maxGrade).toFixed(1)}%`:`${(toStart/1000).toFixed(1)}km\u5148 \u30fb ${Number(next.distanceKm).toFixed(1)}km / \u5e73\u5747 ${Number(next.avgGrade).toFixed(1)}%`;}else els.rideNextClimb.textContent='\u3053\u306e\u5148\u306b\u307e\u3068\u307e\u3063\u305f\u4e0a\u308a\u306f\u3042\u308a\u307e\u305b\u3093';
  updateTurnGuidance(plan,prog,progressM,remainingM,off);
}

function updateRideFollowButton(){
  if(!els.centerRideBtn)return;
  if(state.ride.status!=='recording'){els.centerRideBtn.textContent='現在地';els.centerRideBtn.classList.remove('follow-active','follow-paused');return;}
  const paused=state.ride.followPauseUntil>Date.now();
  els.centerRideBtn.textContent=paused?'追従再開':'追従 ON';
  els.centerRideBtn.classList.toggle('follow-active',!paused);
  els.centerRideBtn.classList.toggle('follow-paused',paused);
}
function pauseRideMapFollow(){
  clearTimeout(state.ride.followTimer);
  state.ride.followPauseUntil=Date.now()+10000;
  updateRideFollowButton();
  state.ride.followTimer=setTimeout(()=>{
    state.ride.followPauseUntil=0;
    updateRideFollowButton();
    if(state.ride.status==='recording'&&state.currentPosition){followRideMap(state.currentPosition,true);showToast('地図の自動追従を再開しました');}
  },10000);
}
function followRideMap(p,force=false){
  if(!state.rideMap||!p||!Number.isFinite(Number(p.lat))||!Number.isFinite(Number(p.lng)))return;
  if(state.ride.status!=='recording'&&!force)return;
  if(!force&&state.ride.followPauseUntil>Date.now())return;
  if(state.ride.followPauseUntil){state.ride.followPauseUntil=0;clearTimeout(state.ride.followTimer);state.ride.followTimer=null;updateRideFollowButton();}
  const here=L.latLng(Number(p.lat),Number(p.lng));
  const bounds=state.rideMap.getBounds();
  const inner=bounds?.isValid?.()?bounds.pad(-0.27):null;
  if(force||!inner||!inner.contains(here)){
    state.rideMap.panTo(here,{animate:true,duration:.35,easeLinearity:.25});
  }
}
...
function onRidePosition(pos){
  ...
  if(state.rideMap){if(state.rideCurrentMarker)state.rideCurrentMarker.setLatLng([p.lat,p.lng]);else state.rideCurrentMarker=L.circleMarker([p.lat,p.lng],{radius:8,color:'#102f53',weight:3,fillColor:'#fff',fillOpacity:1}).addTo(state.rideMap);followRideMap(p)}
...
function resetRideState(){stopPositionWatch();clearInterval(state.ride.timer);clearTimeout(state.ride.followTimer);try{window.speechSynthesis?.cancel()}catch{}releaseWakeLock();state.ride={status:'ready',watchId:null,startTs:0,pauseStarted:0,pausedMs:0,points:[],distanceM:0,ascentM:0,maxSpeedKmh:0,lastAccepted:null,timer:null,wakeLock:null,followPauseUntil:0,followTimer:null};els.liveSpeed.textContent='0.0';els.liveDistance.textContent='0.00 km';els.liveDuration.textContent='00:00:00';els.liveAvgSpeed.textContent='0.0 km/h';els.liveAscent.textContent='0 m';els.liveMaxSpeed.textContent='0.0 km/h';els.rideStateTitle.textContent='READY';els.startRideBtn.textContent='START';els.startRideBtn.disabled=false;els.pauseRideBtn.disabled=true;els.pauseRideBtn.textContent='PAUSE';els.finishRideBtn.disabled=true;updateRideFollowButton();if(state.rideTrackLayer){state.rideTrackLayer.remove();state.rideTrackLayer=null}updatePlanNavigation(state.currentPosition)}

els.startRideBtn.addEventListener('click',async()=>{
  if(state.ride.status==='paused'){state.ride.pausedMs+=Date.now()-state.ride.pauseStarted;state.ride.pauseStarted=0;state.ride.status='recording';state.ride.followPauseUntil=0;clearTimeout(state.ride.followTimer);state.ride.followTimer=null;els.rideStateTitle.textContent='RIDING';els.startRideBtn.disabled=true;els.pauseRideBtn.disabled=false;els.pauseRideBtn.textContent='PAUSE';updateRideFollowButton();startPositionWatch();acquireWakeLock();if(state.currentPosition)followRideMap(state.currentPosition,true);if(state.navVoice.enabled)speakNav('\u97f3\u58f0\u6848\u5185\u3092\u518d\u958b\u3057\u307e\u3059',true);return}
  if(state.ride.status!=='ready')return;
  try{await getCurrentPosition(false)}catch(e){showToast(e.message);return}
  state.ride.status='recording';resetNavVoiceAlerts();state.ride.startTs=Date.now();state.ride.points=[];state.ride.distanceM=0;state.ride.ascentM=0;state.ride.maxSpeedKmh=0;state.ride.lastAccepted=null;state.ride.followPauseUntil=0;clearTimeout(state.ride.followTimer);state.ride.followTimer=null;els.rideStateTitle.textContent='RIDING';els.startRideBtn.disabled=true;els.pauseRideBtn.disabled=false;els.finishRideBtn.disabled=false;updateRideFollowButton();state.ride.timer=setInterval(tickRide,1000);tickRide();startPositionWatch();acquireWakeLock();if(state.rideMap){state.rideMap.invalidateSize({animate:false});syncRideMapView({preferCurrent:true});if(state.currentPosition)followRideMap(state.currentPosition,true);}if(state.navVoice.enabled)speakNav('\u97f3\u58f0\u6848\u5185\u3092\u958b\u59cb\u3057\u307e\u3059',true);showToast('\u8d70\u884c\u8a18\u9332\u3092\u958b\u59cb\u3057\u307e\u3057\u305f\u30fb\u5730\u56f3\u8ffd\u5f93ON');
});
els.pauseRideBtn.addEventListener('click',()=>{if(state.ride.status!=='recording')return;state.ride.status='paused';state.ride.pauseStarted=Date.now();clearTimeout(state.ride.followTimer);state.ride.followTimer=null;state.ride.followPauseUntil=0;stopPositionWatch();try{window.speechSynthesis?.cancel()}catch{}releaseWakeLock();els.rideStateTitle.textContent='PAUSED';els.startRideBtn.disabled=false;els.startRideBtn.textContent='RESUME';els.pauseRideBtn.disabled=true;updateRideFollowButton();showToast('一時停止しました');});
els.finishRideBtn.addEventListener('click',async()=>{if(!['recording','paused'].includes(state.ride.status))return;if(!confirm('走行を終了して記録を保存しますか？'))return;const elapsed=currentElapsed();stopPositionWatch();clearInterval(state.ride.timer);try{window.speechSynthesis?.cancel()}catch{}await releaseWakeLock();const ride={id:`ride_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,startedAt:new Date(state.ride.startTs).toISOString(),endedAt:new Date().toISOString(),durationS:Math.round(elapsed),distanceM:Math.round(state.ride.distanceM),ascentM:Math.round(state.ride.ascentM),maxSpeedKmh:Number(state.ride.maxSpeedKmh.toFixed(1)),avgSpeedKmh:elapsed>0?Number((state.ride.distanceM/1000/(elapsed/3600)).toFixed(1)):0,points:state.ride.points,plannedDestination:state.plannedRoute?.destination?.label||null,plannedMode:state.plannedRoute?.selection?.mode||null};await RideDB.put(ride);resetRideState();setPlannedRoute(null,false);showToast('走行記録を保存しました');switchTab('log');});
els.centerRideBtn.addEventListener('click',()=>{if(!state.rideMap)return;state.ride.followPauseUntil=0;clearTimeout(state.ride.followTimer);state.ride.followTimer=null;updateRideFollowButton();state.rideMap.invalidateSize({animate:false});if(state.currentPosition){if(state.ride.status==='recording')followRideMap(state.currentPosition,true);else syncRideMapView({preferCurrent:true});}else syncRideMapView({preferCurrent:true});});


els.voiceGuideBtn?.addEventListener('click',()=>{const supported='speechSynthesis'in window&&'SpeechSynthesisUtterance'in window;if(!supported){showToast('\u3053\u306e\u7aef\u672b\u3067\u306f\u97f3\u58f0\u6848\u5185\u3092\u5229\u7528\u3067\u304d\u307e\u305b\u3093');return;}state.navVoice.enabled=!state.navVoice.enabled;try{localStorage.setItem(VOICE_KEY,state.navVoice.enabled?'1':'0')}catch{}updateVoiceButton();if(state.navVoice.enabled){speakNav('\u97f3\u58f0\u6848\u5185\u3092\u958b\u59cb\u3057\u307e\u3059',true);showToast('\u97f3\u58f0\u6848\u5185 ON');}else{try{window.speechSynthesis.cancel()}catch{}showToast('\u97f3\u58f0\u6848\u5185 OFF');}});
...
async function registerSW(){if('serviceWorker'in navigator)try{await navigator.serviceWorker.register('./service-worker.js?v=170')}catch{}}

async function boot(){initMaps();loadHome();loadVoicePreference();restorePlannedRoute();registerSW();RideDB.open().catch(()=>{});renderRideLog();getCurrentPosition(false).then(()=>{drawHomeMarker();updatePlanNavigation(state.currentPosition)}).catch(()=>{});console.info(`YOSHI RIDE v${CONFIG.build}`)}
boot();
})();
