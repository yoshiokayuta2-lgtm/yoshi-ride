...
## v1.5
- RIDE中の地図をGPS現在地へ自動追従。
- 現在地が画面中央エリアから外れそうな時だけ自動パンし、地図のガタつきを抑制。
- 走行中に地図を手でドラッグすると追従を10秒だけ一時停止し、その後自動復帰。
- 「追従 ON / 追従再開」ボタンから即座に現在地へ戻せます。
- フロント側のみの更新で、Supabase Edge Functionの再デプロイは不要です。

## v1.6.0 実走フィードバック改善
- RIDEをサイコン型1画面レイアウトへ変更。地図上に現在速度・残距離・ETA・この先の斜度をオーバーレイ表示。
- スマホRIDEではヘッダーを省略し、地図と主要数値をスクロールせず同時確認しやすくした。
